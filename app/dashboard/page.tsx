  ...
  return (
    <div style={{ display:'flex', height:'100vh', fontFamily:F, background:bg, transition:'background .3s' }}>
      <style>{`
        ...
        .stat-num{animation:${animatedStats?'countup .5s ease forwards':'none'}}
              ...
              {campaignStep === 'write' ? (
                ...
                <>
                  <div style={{ marginBottom:16 }}>
                    ...
                    <input value={campaignName} onChange={e => setCampaignName(e.target.value)} placeholder="e.g. Vice Pant Restock — March" style={{ width:'100%', padding:'10px 14px', borderRadius:10, border:`1.5px solid ${border}`, fontSize:13, fontFamily:F, outline:'none', color:text, background:bg }}/>
                  </div>
      ...
      <nav style={{ width:228, background:surface, borderRight:`1px solid ${border}`, display:'flex', flexDirection:'column', height:'100vh', flexShrink:0, transition:'background .3s' }}>
        ...
        <div style={{ padding:'12px 10px', borderTop:`1px solid ${border}` }}>
          <div style={{ display:'flex', alignItems:'center', gap:7, padding:'8px 12px', borderRadius:9, background:'rgba(82,183,136,0.07)', border:'1px solid rgba(82,183,136,0.15)', marginBottom:8 }}>
            <div style={{ width:6, height:6, borderRadius:'50%', background:'#52b788', boxShadow:'0 0 0 2px rgba(82,183,136,0.3)', animation:'pulse-ring 2s ease infinite' }}></div>
            <div style={{ fontSize:11.5, fontWeight:500, color:'#52b788' }}>Operator running</div>
          </div>
          <button onClick={() => setDarkMode(!darkMode)} style={{ width:'100%', padding:'7px', borderRadius:9, cursor:'pointer', fontSize:12, fontWeight:500, fontFamily:F, background:bg, color:muted, border:`1px solid ${border}`, marginBottom:6, transition:'all .2s' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        <div style={{ height:56, background:surface, borderBottom:`1px solid ${border}`, padding:'0 28px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, transition:'background .3s' }}>
          <div>
          ...
          {activeTab==='dashboard' && (
            <div style={{ flex:1, overflow:'auto', padding:'24px 28px' }}>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:22 }}>
                {[
                  { val: contacts.length, label:'Total Contacts', sub:'All time', color:'#0e0e10', dark:'#e8e8f0' },
                  { val: '$'+collected.toLocaleString(), label:'Revenue Collected', sub:'Paid invoices', color:'#52b788', dark:'#52b788' },
                  { val: campaigns.filter(c=>c.status==='live').length, label:'Live Campaigns', sub:'Currently active', color:'#4f8ef7', dark:'#4f8ef7' },
                  { val: messages.length, label:'Messages Handled', sub:'By ops.ai', color:'#f4a237', dark:'#f4a237' },
                ].map((s,i) => (
                  <div key={i} className="stat-num" style={{ background:surface, border:`1px solid ${border}`, borderRadius:14, padding:'18px 20px', position:'relative', overflow:'hidden', animationDelay:`${i*0.1}s` }}>
                    <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background: darkMode ? s.dark : s.color, opacity:.6, borderRadius:'14px 14px 0 0' }}></div>
                    <div style={{ fontSize:30, fontWeight:700, letterSpacing:-1, color: darkMode ? s.dark : s.color, marginBottom:4, marginTop:4 }}>{s.val}</div>
            ...
            <div style={{ flex:1, display:'flex', overflow:'hidden' }}>
              <div style={{ width:290, borderRight:`1px solid ${border}`, background:surface, overflow:'auto', flexShrink:0 }}>
                ...
                {contactThreads.map((c,i) => (
                  <div key={i} onClick={() => loadThread(c)} style={{ padding:'14px 18px', borderBottom:`1px solid ${darkMode?'#1f1f28':'#f4f4f7'}`, cursor:'pointer', background: selectedContact?.id===c.id ? (darkMode?'rgba(79,142,247,0.08)':'rgba(79,142,247,0.05)') : surface, borderLeft: selectedContact?.id===c.id ? '3px solid #4f8ef7' : '3px solid transparent', transition:'all .12s' }}>
                    ...
                    <div style={{ marginBottom:5, display:'flex', alignItems:'center', gap:6 }}>
                      ...
                      <button onClick={e => { e.stopPropagation(); setSlideContact(c) }} style={{ fontSize:9.5, color:muted, background:'none', border:'none', cursor:'pointer', fontFamily:F, padding:'1px 6px', borderRadius:4, background:darkMode?'rgba(255,255,255,0.05)':'rgba(0,0,0,0.04)' }}>Profile</button>
                    </div>
            ...
            <div style={{ flex:1, overflow:'auto', padding:'24px 28px' }}>
              ...
              <div style={card()}>
                ...
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  ...
                  <tbody>{invoices.map((inv,i)=><tr key={i} className="fade-in" style={{ borderBottom:`1px solid ${darkMode?'#1f1f28':'#f4f4f7'}`, animationDelay:`${i*0.05}s` }}><td style={{ padding:'13px 18px' }}><div style={{ display:'flex', alignItems:'center', gap:9 }}><div style={{ width:30, height:30, borderRadius:8, background:'rgba(79,142,247,0.12)', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:12, color:'#4f8ef7', flexShrink:0 }}>{inv.client_name?.[0]}</div><div style={{ fontSize:13, fontWeight:600, color:text }}>{inv.client_name}</div></div></td><td style={{ padding:'13px 18px', fontSize:12.5, color:muted }}>{inv.service}</td><td style={{ padding:'13px 18px', fontSize:13, fontWeight:700, color:text }}>${Number(inv.amount).toLocaleString()}</td><td style={{ padding:'13px 18px', fontSize:12, color:inv.status==='overdue'?'#f25f5c':muted, fontWeight:inv.status==='overdue'?600:400 }}>{inv.due_at}</td><td style={{ padding:'13px 18px' }}><span style={{ fontSize:10, fontWeight:700, padding:'3px 9px', borderRadius:5, ...sc(inv.status) }}>{inv.status?.toUpperCase()}</span></td></tr>)}</tbody>
                </table>
              </div>
            </div>
          )}
                  ...
                  <div style={{ padding:'20px', display:'flex', flexDirection:'column', gap:16 }}>
                    {[{label:'Business Name',key:'name'},{label:'Owner Name',key:'owner_name'},{label:'Location',key:'location'},{label:'Store Link',key:'booking_link'},{label:'Hours',key:'hours'}].map(field => (
                      <div key={field.key}>
                        ...
                        <input style={{ width:'100%', padding:'10px 14px', borderRadius:10, border:`1.5px solid ${border}`, fontSize:13, outline:'none', fontFamily:F, color:text, background:bg, transition:'border .15s' }} value={business[field.key]||''} onChange={e => setBusiness({...business,[field.key]:e.target.value})}/>
                      </div>
                    ))}
                    <div>
